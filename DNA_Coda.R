                ...
                tuning=c("none","cv"),folds=5)
{
    ...
    return(list(DNA_Coda=ret,lambda=lambda,nlambda=nlambda,opt=opt));
}
...
{
    err <- tr(D%*%S1%*%D%*%S0)/2-tr(S1%*%D)+tr(S0%*%D);
    return(err); 
}
                   ...
                   folds=5)
{
  ...
  losses <- array(NA,c(folds,length(lambda)));
  cat("CV fold:");
  for(i in 1:folds)
  {
    cat("",i);
    ...
    fit <- DNA_Coda(X1[-test1,],X0[-test0,],lambda,length(lambda),NULL,
    ...
    losses[i,] <- sapply(fit$DNA_Coda,loss,S1.test,S0.test);
  }
  cat("\n");
  opt <- which.min(apply(losses,2,mean));
  return(opt);
}
